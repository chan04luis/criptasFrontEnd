...
const IndexClientes: React.FC<IndexClientesProps> = ({ result }) => {
      ...
      <AppBar position="static" sx={{ backgroundColor: parentConfig?.ColorSecundario || "#000000" }}>
        <Toolbar>
          ...
          <Box sx={{ display: "flex", gap: 1 }}>
            {result?.PermisosBotones.find(x => x.ClaveBoton == 'add_client')?.TienePermiso && <CustomIconButton onClick={() => handleOpenModal(null)} title="Agregar">
              <AddIcon />
            </CustomIconButton>}
              ...
              <TableBody>
                {clientes.map((cliente) => (
                  <TableRow key={cliente.Id}>
                    ...
                    <TableCell>{cliente.Origen === 1 ? "Admin UI" : (cliente.Origen === 2 ? "WEB UI" : "APP")}</TableCell>
                    <TableCell>
                      <IconButton
                    ...
                    <TableCell>
                      {cliente.Id != IdCliente ? (
                        <>
                          {result?.PermisosBotones.find(x => x.ClaveBoton == 'edit_client')?.TienePermiso && <IconButton onClick={() => handleOpenModal(cliente)} title="Editar">
                            <EditIcon />
                          </IconButton>}
                          {result?.PermisosBotones.find(x => x.ClaveBoton == 'delete_client')?.TienePermiso && <IconButton onClick={() => handleDeleteCliente(cliente.Id)} title="Eliminar" sx={{ color: "red" }}>
                            <DeleteForeverIcon />
                          </IconButton>}
                        </>
                      ) : <></>}
